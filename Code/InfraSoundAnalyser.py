...
def plotFiltered(Data, samplingFreq, startDT):
    ...
    # Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotFiltered.png'
...
def plotBands(tr):
    ...
    fig.tight_layout()
        ...
        # Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotBands.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotMultiple(tr1, tr2, tr3, tr4, yMin, yMax, lowCut, highCut):
    ...
    # Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotMultiple.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotPeriodogram(tr, fMin, fMax):
    ...
    # Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotPeriodogram.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotSpectrogram(tr, lowCut, highCut):
    ...
    # Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotSpectrogram.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotWaveletTransform(tr):
    ...
    # Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotWaveletTransform.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def DayPlotAcousticPower(tr, lowCut, highCut, deltaT):
    ...
    fig = st2.plot(type="dayplot", title='test', data_unit='$Wm^{-2}$', interval=60, right_vertical_labels=False,
             one_tick_per_line=False, color=['k', 'r', 'b', 'g'], show_y_UTC_label=False)

    #Save figure
    save_path = '/home/adam/Desktop/Wiatraki/DayPlotAcousticPower.png'
...
def PlotAcousticPower(tr, lowCut, highCut):
    ...
    #Save figure
    save_path = '/home/adam/Desktop/Wiatraki/PlotAcousticPower.png'
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotMany():
    ...
    fig = plotMultiple(tr11, tr12, tr13, tr14, yMin, yMax, lowCut, highCut)

    #Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotMany.png'
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotDayplot(tr, lowCut, highCut):
    ...
    fig = tr.plot(type="dayplot", title='my title', data_unit='$\Delta$Pa', interval=60, right_vertical_labels=False,
            one_tick_per_line=False, color=['k', 'r', 'b', 'g'], show_y_UTC_label=False)

    #Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotDayplot.png'
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def simplePlot(tr, lowCut, highCut):
    ...
    #Save figure
    save_path = '/home/adam/Desktop/Wiatraki/simplePlot.png'
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotsSimpleFFT(tr):
    ...
    sigTemp = tr.data #oryginalnie tr2.data
    min_len = min(len(t), len(sigTemp))  # aby zgadzaly sie wymiary t i s
    t = t[:min_len] # dlugosc osi czasu
    s = sigTemp[:min_len]  #dlugosc sygnalu  #[0:len(t)]
    ...
    #f, Pxx = welch(s, fs=Fs, nperseg=256)  # v3
    #axes[2, 1].plot(f, Pxx, color='C3') # v3
    #axes[2, 1].set_yscale('log') # v3
    #[2, 1].set_xlabel("f - Hz") # v3
    #axes[2, 1].set_ylabel("Relative Power Amplitude") # v3
    axes[2, 1].psd(s, 1024, Fs, Fc=1)   #v3 zakomentowane zamiast f,Pxx = ...
    #axes[2, 1].set_xlim(0.0, 6) # ograniczono przedzial aby odczytac czest obrotu smigla
    ...
    #Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotsSimpleFFT.png'
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotsSimpleFFT2(tr):
    ...
    # Tworzenie osi czasu
    t = np.arange(0, len(tr.data) * dt, dt)
    s = tr.data  # Sygnał do analizy

    # Obliczanie widma gęstości mocy (PSD)
    f, Pxx = welch(s, fs=Fs, nperseg=1024)

    # Tworzenie wykresu log-log
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(f, Pxx, label="PSD", color='C0')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlim(0.1, 5)  # Zakres częstotliwości od 0.1 do 5 Hz
    ax.set_xlabel('Frequency (Hz)')
    ax.set_ylabel('Relative Power Amplitude')
    ax.set_title('Wind Turbine: FFT Showing Fundamental and Harmonics')
    ax.grid(True, which='both', linestyle='--', linewidth=0.5)

    # Ustawienie takich znacznikow co 1 Hz
    xticks = np.arange(1,6,1)
    ax.set_xticks(xticks)
    ax.get_xaxis().set_major_formatter(plt.ScalarFormatter()) #bez skali logarytmicznej
    
    # Dodanie oznaczeń f0 i harmonicznych
    f0 = 0.975  # Częstotliwość podstawowa
    harmonic_count = 5  # Liczba harmonicznych do zaznaczenia
    for i in range(1, harmonic_count + 1):
        ax.axvline(f0 * i, color='r', linestyle='--', label=f'f{i}' if i == 1 else None)

    # Dodanie legendy
    ax.legend(loc='upper right')

    # Zapis wykresu
    save_path = '/home/adam/Desktop/Wiatraki/plotsSimpleFFT_modified.png'
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotsSimpleFFT3(tr):
    ...
    freqs, psd = signal.welch(s, fs=Fs, nperseg = 1024)
    axes[2, 1].set_title("Power Spectrum Density (Log Y")
    axes[2, 1].plot(freqs, psd, color='C3')
    axes[2, 1].set_xscale('linear')
    axes[2, 1].set_yscale('log')
    axes[2, 1].set_xlabel("Frequency - Hz")
    axes[2, 1].set_ylabel("Relative Power Amplitude")
    ...
    #Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotsSimpleFFT3.png'
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
def plotmagnitudeSpectrum(tr):
    ...
    t = np.arange(startSec, endSec, dt)  # create np array for time axis
    sigTemp = tr2.data
    min_len = min(len(t), len(sigTemp))  # to samo co wyzej zeby t i s mialy odpowiadajace sobie wymiary
    t = t[:min_len]
    s = sigTemp[0:min_len]#0:len(t)]
    ...
    #Save figure
    save_path = '/home/adam/Desktop/Wiatraki/plotmagnitudeSpectrum.png'
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Wykres zapisano w: {save_path}")
...
lowCut = 0.1  # low frequency cut-off
highCut =20  # highfrequency cut-off
...
startMinute = 390#2.5  #fajerwerki np 341-342
endMinute = 450#3.5  
tracestart = tr1.stats.starttime
startSec = (startMinute * 60)
endSec = (endMinute * 60)
tr2 = tr1.trim(tracestart + startSec, tracestart + endSec)
#------------------------
...
plotsSimpleFFT2(tr2)
plotsSimpleFFT3(tr2)
# plotsSimpleFFT3(tr2, freq_ranges= [(0.1, 6.0)])
