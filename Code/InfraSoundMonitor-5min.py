...
# zmodyfikowana wersja kodu przystosowana do zapisu 5-minutowych próbek danych
...
def save_hourly_data_as_mseed(st):
	...
	minute = str(start_date_time.minute)
	
	save_dir=  'Data' + '/' + year + '/' + month+ '/' + day
	filename = hour + '_' + minute + '.mseed'
...
os.chdir('/home/adam/Desktop/Wiatraki')  #replace with home directory of application
...
n_target_hourly_samples=int(frq_sampling*300*1.1) # 5 minut
n_target_daily_samples=int(n_target_hourly_samples*24*12) # bo w godzinie jest 12 5-minutowych przedziałów
...
while 1:
	...
	if (UTCDateTime() - hourly_start_time >= 5 * 60):  # Sprawdź, czy upłynęło 5 minut
		sample_end_time = UTCDateTime()
    
        # Uruchom zapis i przetwarzanie danych w osobnym wątku
		...
		hourly_start_time = UTCDateTime()  # Ustaw nowy początek 5-minutowego interwału
		hourly_readings = np.zeros(n_target_hourly_samples, np.float32)  # Reset danych dla kolejnych 5 minut
		n_hourly_samples = 0
